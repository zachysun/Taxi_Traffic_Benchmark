...
from trainer.trainer_astgcn import trainer
...
def main(config):
    ...
    for data in data_x_sets:
        ...
        data_normed = np.einsum('btnf->bnft', data_normed)
    ...
    model = make_model(data_config['device'], trainer_config['nb_block'], trainer_config['in_channels'],
                       trainer_config['K'], trainer_config['nb_chev_filter'], trainer_config['nb_time_filter'],
                       trainer_config['num_of_hours'], origin_adj, data_config['output_seq'],
                       data_config['input_seq'], data_config['num_of_vertices'])
    model.to(device)
    ...
    torch.save(model.state_dict(), trainer_config['model_path'] + '/' + 'astgcn_pretrained' + '.pth')
    print('Model saved!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='../config/config_astgcn.yaml',
                        help='Config path')
