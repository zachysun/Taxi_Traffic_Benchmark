...
from models.stgcn.models import STGCNChebGraphConv
from utils import ProcessData, StandardScalar
from torch.utils.data import DataLoader, TensorDataset
from utils import masked_mae, calculate_scaled_laplacian_torch
from trainer.trainer_stgcn import trainer
...
def main(config):
    ...
    for data in data_x_sets:
        ...
        data_normed = np.einsum('btnf->bftn', data_normed)
...
# ------------------------ Model setting ------------------------
    ...
    Ko = trainer_config['n_his'] - (trainer_config['Kt'] - 1) * 2 * trainer_config['stblock_num']
    blocks = [[trainer_config['channel']]]
    for l in range(trainer_config['stblock_num']):
        blocks.append([64, 16, 64])
    if Ko == 0:
        blocks.append([128])
    elif Ko > 0:
        blocks.append([128, 128])
    blocks.append([trainer_config['channel']])

    gso = calculate_scaled_laplacian_torch(origin_adj)

    model = STGCNChebGraphConv(blocks, data_config['num_of_vertices'], trainer_config['Kt'], trainer_config['Ks'],
                               trainer_config['act_func'], trainer_config['graph_conv_type'], gso
                               ,trainer_config['enable_bias'], trainer_config['dropout'], trainer_config['n_his'])
    model.to(device)
    ...
    torch.save(model.state_dict(), trainer_config['model_path'] + '/' + 'stgcn_pretrained' + '.pth')
    print('Model saved!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='../config/config_stgcn.yaml',
                        help='Config path')
