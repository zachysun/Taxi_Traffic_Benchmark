    ...
    def forward(self, x, spatial_attention):
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                # temp = torch.einsum('nvv,nvc->nvc', (T_k_with_at, graph_signal))
                # b = (rhs == temp)
...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials,
        ...
        self.final_conv = nn.Conv2d(int(len_input / time_strides), num_for_predict,
                                    kernel_size=(1, nb_time_filter - in_channels + 1))
...
def make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict,
    ...
    :return:
    """
    L_tilde = calculate_scaled_laplacian_torch(adj_mx)
    cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(DEVICE) for i in get_Tk(L_tilde, K)]
